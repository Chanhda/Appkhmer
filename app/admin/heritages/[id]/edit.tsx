    ...
    async function loadHeritage() {
      ...
      try {
        ...
        if (item) {
          ...
          Alert.alert('Lỗi', 'Không tìm thấy di sản.');
          router.back();
        }
      } catch (error) {
        console.error(error);
        Alert.alert('Lỗi', 'Không thể tải dữ liệu di sản.');
        router.back();
      } finally {
        setLoading(false);
      }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        Alert.alert('Thành công', 'Đã cập nhật thông tin di sản.');
      }
      router.back();
    } catch (error) {
      console.error(error);
      const msg = error instanceof Error ? error.message : 'Lỗi không xác định';
      Alert.alert('Lỗi', `Không thể cập nhật di sản: ${msg}`);
    } finally {
      setIsSaving(false);
    }
  }
